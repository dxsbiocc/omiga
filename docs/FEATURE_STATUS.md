...
| Multi-agent workflows | beta | `src-tauri/src/domain/agents/scheduler/*`, `src-tauri/src/domain/agents/background.rs`, `src-tauri/src/commands/chat/subagent.rs`, `src-tauri/src/domain/persistence/mod.rs` | Add clearer task lifecycle UI and multi-agent validation. |
| Repository-aware context | stable | `src-tauri/src/domain/session/config.rs`, `src/components/CodeWorkspace/*`, `src/components/Chat/composerPathMentions.ts` | Keep workspace context lightweight and research-task focused. |
