...
Runnable examples should live in the external plugin repository or be generated by tests at runtime. Current resource plugins are under:

```text
../omiga-plugins/plugins/resources/ (or create a temp plugin in tests)
```

Use those marketplace plugins as canonical examples when authoring or testing retrieval plugins; app-local fixtures are intentionally not a plugin source.
...
| `replacesBuiltin` | no | Set `true` only to intentionally supersede a built-in source that shares this resource's `category.id`. See "Replacing a built-in source" below. |
| `parameters` | no | JSON metadata for resource-specific parameters. |

### Replacing a built-in source

A plugin resource whose `category.id` matches a built-in retrieval source is treated as a conflict and **rejected** unless it sets `replacesBuiltin: true`. When `replacesBuiltin` is set, the built-in source remains the default and the plugin only takes over once its resource is **explicitly enabled** in Settings → Plugins; if the plugin resource is disabled, routing falls back to the built-in. This keeps built-in sources working out of the box while letting a plugin opt in to overriding one.
