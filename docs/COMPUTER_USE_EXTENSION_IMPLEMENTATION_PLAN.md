...
| --- | --- | --- |
| Phase 1 — 插件/MCP 基础设施修正 | Done | `computer-use` external marketplace plugin scaffold；plugin `.mcp.json` relative cwd rebasing test passed |
...
| Phase 23 — Rust sidecar artifact integrity | Done | release check can verify installed external marketplace sidecar existence/executable bit/SHA-256/source-hash match and run the installed binary through Rust mock MCP smoke；install status reports source/destination SHA-256 |
...
| Phase 27 — Final packaging hygiene gate | Done | release check supports `--fail-on-generated-artifacts` to block final packaging when external marketplace plugin resources contain `__pycache__` or `.pyc` files |
| Phase 28 — Generated artifact cleanup | Done | removed generated `__pycache__`/`.pyc` from the external marketplace computer-use plugin and verified `--fail-on-generated-artifacts` passes |
...
1. 确认 external marketplace plugin 使用当前目录结构：
...
../omiga-plugins/marketplace.json
../omiga-plugins/plugins/automation/computer-use/plugin.json
../omiga-plugins/plugins/automation/computer-use/.mcp.json
```
...
src-tauri/src/domain/tools/mod.rs
../omiga-plugins/plugins/automation/computer-use/bin/.../computer-use
```
...
```text
../omiga-plugins/plugins/automation/computer-use/bin source or sidecar crate
...
`type_text`, and disabled clipboard fallback for sensitive input. The external-marketplace
...
external marketplace plugin as `bin/computer-use-sidecar`. `.gitignore` ignores the installed
...
copy target/release/computer-use-sidecar
  ↓
../omiga-plugins/plugins/automation/computer-use/bin/computer-use-sidecar
  ↓
chmod 755
```
...
computer_use_backend_status
  ↓
inspect external marketplace computer-use plugin paths
  ↓
return status JSON
```
...
补齐 Rust opt-in 发布路径中的“安装产物是否就是刚构建/指定的产物”验证。保持默认 runtime 为 Python，不改变插件启用语义，也不把 sidecar 二进制纳入源码跟踪；只在显式 release/package gate 中检查已安装的 external marketplace sidecar。
...
`--verify-installed-sidecar`, the installed external-marketplace binary must be executable
...
进入稳定化阶段后，不继续扩展 Computer Use 功能，而是把发布前最容易遗漏的 packaging 配置纳入现有 release gate。目标是确认 external marketplace plugin 会被 Tauri 打包、marketplace 指向正确 plugin、plugin manifest 与 `.mcp.json` 指向稳定 wrapper、平台 wrapper 有可执行位并通过 shell syntax check。
...
`external marketplace config`, external marketplace entry, `plugin.json`, `.mcp.json`,
wrapper files, executable bits, and wrapper shell syntax. It also reports
generated packaging artifacts such as `__pycache__`/`.pyc` under the external-marketplace
...
```text
src-tauri/tauri.conf.json does not bundle plugin payloads; marketplace points at ../omiga-plugins
../omiga-plugins/marketplace.json has computer-use entry
...
closed when external marketplace plugin resources contain `__pycache__` or `.pyc` files.
...
```text
../omiga-plugins/plugins/automation/computer-use/**/__pycache__
../omiga-plugins/plugins/automation/computer-use/**/*.pyc
```
...
执行最终打包卫生检查前的最小清理：只删除 Computer Use external marketplace plugin 目录下已确认的 Python bytecode 生成物，不触碰源码、不做 bundle/sign/upload、不新增功能。

V28 status: completed. Removed:

```text
../omiga-plugins/plugins/automation/computer-use/bin/__pycache__/computer-use-macos.cpython-312.pyc
../omiga-plugins/plugins/automation/computer-use/bin/__pycache__
```
...
```text
computer-use external marketplace plugin 内的 __pycache__ / .pyc
```
