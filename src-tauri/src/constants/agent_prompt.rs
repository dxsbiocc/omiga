...
- Exact skill tool names are lowercase: `list_skills`, `skill_view`, and `skill`. If a bash call is rejected because a dedicated tool exists, immediately retry with that dedicated tool; do not keep changing shell quoting.
...
- Do not use `bash` for file reading/searching/writing when `file_read`, `ripgrep`, `file_write`, `file_edit`, or `notebook_edit` can express the action.
