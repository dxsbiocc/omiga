...
static SECURITY_WEAK: &[&str] = &["权限检查", "敏感信息", "密钥泄露"];
...
pub fn classify(message: &str) -> TaskIntent {
    ...
    let scores: [(usize, TaskIntent); 11] = [
        (
            score(&m, SECURITY_STRONG, SECURITY_WEAK),
            TaskIntent::Security,
        ),
        (
            score(&m, DEBUGGING_STRONG, DEBUGGING_WEAK),
            TaskIntent::Debugging,
        ),
        (
            score(&m, CODE_REVIEW_STRONG, CODE_REVIEW_WEAK),
            TaskIntent::CodeReview,
        ),
        (
            score(&m, ARCHITECTURE_STRONG, ARCHITECTURE_WEAK),
            TaskIntent::Architecture,
        ),
        (score(&m, TESTING_STRONG, TESTING_WEAK), TaskIntent::Testing),
        (
            score(&m, RESEARCH_STRONG, RESEARCH_WEAK),
            TaskIntent::Research,
        ),
        (
            score(&m, DATA_ANALYSIS_STRONG, DATA_ANALYSIS_WEAK),
            TaskIntent::DataAnalysis,
        ),
        (
            score(&m, PLANNING_STRONG, PLANNING_WEAK),
            TaskIntent::Planning,
        ),
        (
            score(&m, VERIFICATION_STRONG, VERIFICATION_WEAK),
            TaskIntent::Verification,
        ),
        (
            score(&m, PERFORMANCE_STRONG, PERFORMANCE_WEAK),
            TaskIntent::Performance,
        ),
        (
            score(&m, REFACTORING_STRONG, REFACTORING_WEAK),
...
mod tests {
    ...
    fn security_keywords() {
        assert_eq!(
            classify("check for sql injection vulnerabilities"),
            TaskIntent::Security
        );
        assert_eq!(
            classify("security audit of auth module"),
            TaskIntent::Security
        );
        assert_eq!(
            classify("scan for xss vulnerabilities"),
            TaskIntent::Security
        );
        assert_eq!(
            classify("there is a credential leak in the config"),
            TaskIntent::Security
        );
    }
    ...
    fn security_beats_code_review_on_overlap() {
        ...
        assert_eq!(
            classify("review the security audit of auth"),
            TaskIntent::Security
        );
    ...
    fn security_false_positive_guard() {
        assert_eq!(
            classify("I have a secret project idea"),
            TaskIntent::General
        );
        assert_eq!(
            classify("survey the codebase structure"),
            TaskIntent::General
        );
    ...
    fn debug_keywords() {
        ...
        assert_eq!(
            classify("why is this crashing on startup"),
            TaskIntent::Debugging
        );
        assert_eq!(
            classify("stack trace: thread main panicked"),
            TaskIntent::Debugging
        );
    }
    ...
    fn code_review_keywords() {
        ...
        assert_eq!(
            classify("do a code review of main.rs"),
            TaskIntent::CodeReview
        );
    }
    ...
    fn architecture_keywords() {
        assert_eq!(
            classify("design the system architecture for a microservice"),
            TaskIntent::Architecture
        );
        assert_eq!(
            classify("what are the tradeoffs of this design pattern"),
            TaskIntent::Architecture
        );
    }
    ...
    fn testing_keywords() {
        assert_eq!(
            classify("write unit tests for the auth module"),
            TaskIntent::Testing
        );
        assert_eq!(
            classify("add integration tests for the api"),
            TaskIntent::Testing
        );
    }
    ...
    fn research_keywords() {
        assert_eq!(
            classify("find papers on transformer attention"),
            TaskIntent::Research
        );
        assert_eq!(
            classify("literature survey on diffusion models"),
            TaskIntent::Research
        );
    }
    ...
    fn data_analysis_keywords() {
        assert_eq!(
            classify("analyze this dataset with pandas"),
            TaskIntent::DataAnalysis
        );
        assert_eq!(
            classify("run a correlation analysis"),
            TaskIntent::DataAnalysis
        );
    }
    ...
    fn performance_keywords() {
        assert_eq!(
            classify("this page is really slow, help me optimize"),
            TaskIntent::Performance
        );
        assert_eq!(
            classify("there's a memory leak in the server"),
            TaskIntent::Performance
        );
    ...
    fn refactoring_keywords() {
        assert_eq!(
            classify("refactor this module to reduce duplication"),
            TaskIntent::Refactoring
        );
        assert_eq!(
            classify("clean up this code and extract functions"),
            TaskIntent::Refactoring
        );
    }
    ...
    fn planning_keywords() {
        assert_eq!(
            classify("plan before we implement the new feature"),
            TaskIntent::Planning
        );
    }
    ...
    fn verification_keywords() {
        assert_eq!(
            classify("verify that the auth flow works correctly"),
            TaskIntent::Verification
        );
    }
    ...
    fn general_fallback() {
        assert_eq!(
            classify("add a new button to the settings page"),
            TaskIntent::General
        );
