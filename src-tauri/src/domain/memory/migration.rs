...
async fn migrate_wiki(old_wiki: &Path, new_wiki: &Path) -> Result<(), AppError> {
    fs::create_dir_all(new_wiki).await
        .map_err(|e| AppError::Unknown(format!("Failed to create new wiki dir: {}", e)))?;
    
    // Copy all .md files
    let mut entries = fs::read_dir(old_wiki).await
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        if path.extension().map(|e| e == "md").unwrap_or(false) {
            let file_name = path.file_name().ok_or_else(|| {
                AppError::Unknown("Invalid file name".to_string())
            })?;
            let dest = new_wiki.join(file_name);
            fs::copy(&path, &dest).await
                .map_err(|e| AppError::Unknown(format!("Failed to copy {}: {}", file_name.to_string_lossy(), e)))?;
...
async fn migrate_pageindex(old_memory: &Path, new_implicit: &Path) -> Result<(), AppError> {
    fs::create_dir_all(new_implicit).await
    ...
    if old_content.exists() {
        ...
        fs::rename(&old_content, &new_content).await
...
mod tests {
    ...
    async fn test_detect_unified() {
        ...
        let config_path = temp.path().join(".omiga").join("memory").join("config.json");
        fs::create_dir_all(config_path.parent().unwrap()).await.unwrap();
    ...
    async fn test_detect_legacy_wiki() {
        let temp = TempDir::new().unwrap();
        fs::create_dir_all(temp.path().join(".omiga").join("wiki")).await.unwrap();
    ...
    async fn test_detect_none() {
        let temp = TempDir::new().unwrap();
        fs::create_dir_all(temp.path().join(".omiga")).await.unwrap();
