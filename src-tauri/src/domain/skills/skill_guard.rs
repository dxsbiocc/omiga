...
mod tests {
    ...
    fn shell_rc_mod_is_caution() {
        ...
        assert!(result.findings.iter().any(|f| f.pattern_id == "shell_rc_mod"));
    }
    ...
    fn eval_base64_is_obfuscation() {
        ...
        assert_eq!(result.verdict, Verdict::Dangerous);
        assert!(result.findings.iter().any(|f| f.category == Category::Obfuscation));
    }
