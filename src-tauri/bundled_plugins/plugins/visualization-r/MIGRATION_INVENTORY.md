...
| `scatter/dumbbell_region` | `viz_scatter_region_dumbbell` | `templates/scatter/region-dumbbell` | Migrated from JSON DSL to direct ggplot2 region dumbbell plot with mean ± SD background bands, mean lines, paired category links, and endpoint dots; `data.json` converted to TSV. |
| `scatter/jitter_group` | `viz_scatter_group_jitter` | `templates/scatter/group-jitter` | Migrated from JSON DSL to direct ggplot2 grouped differential jitter scatter with category bands, background extent columns, change colors, and top labels; `data.json` converted to TSV. |
| `scatter/lollipop_polar` | `viz_scatter_polar_lollipop` | `templates/scatter/polar-lollipop` | Migrated from JSON DSL to direct ggplot2 polar lollipop plot with radial guide rings, grouped endpoint colors, outside category labels, and value labels; `data.json` converted to TSV. |
| `scatter/lollipop_radial` | `viz_scatter_radial_lollipop` | `templates/scatter/radial-lollipop` | Migrated from JSON DSL to direct ggplot2 radial fan lollipop plot with grouped endpoint colors, guide rings, and coord_radial donut center; `data.json` converted to TSV. |
| `scatter/marginal` | `viz_scatter_marginal` | `templates/scatter/marginal` | Migrated from JSON DSL plus ggExtra marginal layer to direct ggplot2 scatter with linear fit, correlation label, and grouped marginal boxplots; `data.json` converted to TSV. |
| `scatter/one2many` | `viz_scatter_one_to_many` | `templates/scatter/one-to-many` | Migrated from JSON DSL plus ggforce Bezier layer to direct ggplot2 one-to-many correlation plot with generated cubic Bezier paths, category-filled endpoints, size-scaled correlations, and direction arrows; `data.json` converted to TSV. |
| `scatter/paired` | `viz_scatter_paired` | `templates/scatter/paired` | Migrated from JSON DSL plus geom_signif to direct ggplot2 paired scatter/line plot with matched-sample links, condition-filled points, and base-R paired significance brackets; `data.json` converted to TSV. |
| `scatter/rank` | `viz_scatter_rank` | `templates/scatter/rank` | Migrated from JSON DSL to direct ggplot2 ranked feature scatter with effect-size color gradient, absolute-effect size scaling, highlighted rank extremes, and optional ggrepel labels; `data.json` converted to TSV. |
